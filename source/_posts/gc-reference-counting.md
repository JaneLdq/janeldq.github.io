...
* **局部变量引用（variable reference）不计入引用计数**，也就是说那些只被本地变量引用的对象不在延迟引用计数法管理范围内，默认他们随着栈弹出消亡。
* **用哈希表保存对象的引用数**。
...
（想了半天我也没明白为什么要叫做 “sticky” 引用计数法，sticky到底是什么意思呢？是指什么都不做的情况下那些不再被处理的垃圾就被像 stick 在内存空间里一样了吗...）
