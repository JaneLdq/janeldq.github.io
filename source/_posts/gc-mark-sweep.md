...
---
接下来，就进入本文的正题 —— **标记-清除算法**。

顾名思义，标记-清除 (Mark-Sweep) 算法分为两个阶段：
* **标记阶段** - 先标记出所有需要回收的对象
* **清除阶段** - 统一回收所有被标记的对象

那么问题来了，**哪些对象是需要回收的呢？** 这就涉及到一个判定方法 —— 可达性分析。
...
* 标记阶段会标记所有的存活对象，由此可知，**标记时间与存活对象的总数成正比**

---
# 清除阶段
在清除阶段，GC会便利整个堆，回收未被标记的对象（即垃圾们）。
伪码如下：
```java
sweep_phase () {
...
### 碎片化 ragmentation)
上文也提到了，标记-清除算法在使用过程中会逐步被细化的分块。
* 如果碎片化严重，即时堆中分块的总大小够用，也会因为每个分块太小而不能执行分配。
* 一般在将数据从内存读取到缓存中时，考虑到“访问的局限性”（即具有引用关系的对象之间很可能存在连续访问的情况），会将其附近的数据读取到缓存中。如果能把具有引用关系的对象安排在邻近位置则能提高在缓存中读取到想利用的数据的概率。而碎片化很可能导致对象相隔太远，降低命中率。

### 分配速度受限
由于分块不是连续的，每次分配都必须遍历空闲链表，找到足够大的分块。最坏的情况时每次进行分配都要把空闲链表遍历到最后。
...
正因为这种特性，标记-清除算法每次进行标记时都要对对象进行写入操作，从而触发“写时复制”，这样会导致频繁发生本不应该的复制。

---
# 优化
针对上文指出的缺点，我们针对性的采取一些优化措施。

## 多个空闲链表
上面提到使用单个空闲链表，由于空间的不连续性，每次都有可能要遍历整个链表来找到合适大小的分块。针对这一情况，我们可以按照分块大小创建多个空闲链表。比如创建一个分块大小全是 2 个字长的空闲链表，一个分块大小全是 3 个字长的空闲链表，...，一个分块大小全是 N 个字长的空闲链表。
...
这样做的目的原本是为了消除碎片化，但效果并不好。比如如果在全部用于 2 个字长的块中只有一个两个存活对象，其实大多数的 2 个字长的分块仍然是分散而未能使用的碎片。

---
## 位图标记
前面提到的标记 是直接作用在对象上，另一种标记方式则是将对象的标志位单独拎出来，与对象分开管理。通常我们称这组标志位的集合为**“位图表格 (bitmap table)”**。利用这个表进行标记的行为就被称为**“位图标记”**。
位图表格的实现方式有很多，数组、散列表、树形结构等都行，只要能保证每一“位”能与堆中的对象一一对应即可。一般来说，堆中的 1 个字会分配到 1 个“位”。
...
延迟清除 (Lazy Sweep) 是针对缩减清除操作所用时间的优化。主要改进就是在标记结束后，不一定立马进行清除操作，而是延迟到下一次为新对象分配空间时。
...
* 如果延迟清除的指针进入了大批存活对象所处的范围，则很可能一直查询失败，直到遍历完所有的存活对象才找到合适的空闲分块
